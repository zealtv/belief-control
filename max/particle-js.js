...
var compass = 1; // 0-360 deg from weather station
...
var currentState = 'flock'; // default state
...
// ---- State switching
function updateState(v) //update between flow and flock mode
{
	currentState = v;
	if (soundObjects.length <= 0) return;

	soundObjects.forEach(function(soundObject) {
		soundObject.rules = state.states[currentState].rules;
	});
	post("State: " + currentState + "\n");
}
...
function SoundObject() {
	...
	this.x = (state.currentState === 'flow') ? 0 : Math.random();
	this.y = (state.currentState === 'flow') ? 0.5 : Math.random();
	this.vx = 0;
	this.vy = 0;
	...
	this.rules = state.states[currentState].rules //get the rules from state
...
function sep(v)
{
	separation = clip(v,0,1);
}

function ali(v)
{
	alignment = clip(v,0,1);
}

function coh(v)
{
	coherence = clip(v,0,1);
}

function septhresh(v)
{
	separationThreshold = clip(v,0,1);
}
...
function align(s)
{
	...
	compassRadians = (compass/360) * 2 * Math.PI; //to radians
	// post(compassRadians +"\n");
...
{
	compass = v; //wind direction 0-360
}
